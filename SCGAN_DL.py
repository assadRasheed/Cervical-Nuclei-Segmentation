...
def weighted_bce_loss(y_true, y_pred, weight=0.5):
    ...
    return weight * bce + (1 - weight) * (1 - bce)

def hd_loss(y_true, y_pred):
    # Hausdorff Distance Loss placeholder (requires complex implementation)
    return tf.reduce_mean(tf.abs(y_true - y_pred))  # Placeholder for actual HD loss
...
# ========================== SCGAN Model for Flexible Loss Function ==========================
class SCGAN(tf.keras.Model):
    def __init__(self, generator, discriminator, loss_type='BCE'):
        ...
        # Set the loss function based on the selected type
        if loss_type == 'Dice':
            self.loss_function = dice_loss
        elif loss_type == 'wBCE':
            self.loss_function = lambda y_true, y_pred: weighted_bce_loss(y_true, y_pred)
        elif loss_type == 'L1':
            self.loss_function = self.mae
        elif loss_type == 'HD':
            self.loss_function = hd_loss
        else:  # Default to BCE
...
def train_progressive_resizing(initial_model, dataset, image_sizes, epochs_per_size=5, loss_type='BCE'):
    models = []
    ...
    for size in image_sizes:
        ...
        # Create SCGAN with new generator and specified loss function
        discriminator = build_discriminator(input_shape=(size, size, 1))
        scgan = SCGAN(new_model, discriminator, loss_type=loss_type)
...
# trained_models = train_progressive_resizing(initial_generator, real_dataset, image_sizes, loss_type='Dice')